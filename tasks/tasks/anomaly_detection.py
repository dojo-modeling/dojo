...
def main():
    ...
    with torch.no_grad():
        ...
        plt.scatter(
            anomalous_scores.cpu(),
            np.ones(anomalous_scores.shape[0]) * 0,
            c="r",
            s=200,
            marker="^",
        )
...
class Encoder(nn.Module):
    def __init__(self, in_channels, latent_channels):
        ...
        self.final = nn.Linear(
            in_features=working_channels, out_features=latent_channels
        )
...
class Decoder(nn.Module):
    def __init__(self, latent_channels, out_channels):
        ...
        self.embed = nn.Linear(
            in_features=latent_channels, out_features=working_channels
        )
...
class AnomalyDetector(nn.Module):
    def __init__(self, latent_channels=32, root_dir="models/autoencoder"):
        ...
        self.scores_path = join(
            root_dir, "scores.pt"
        )  # scores of all data that was trained. for generating percentile scores
    ...
    def load_autoencoder(self, verbose=True):
        ...
        if exists(self.encoder_path) and exists(self.decoder_path):
            if verbose:
                print(f"loading saved auto-encoder model from {self.root_dir}")
            self.encoder.load_state_dict(
                torch.load(self.encoder_path, map_location=torch.device("cpu"))
            )
            self.decoder.load_state_dict(
                torch.load(self.decoder_path, map_location=torch.device("cpu"))
            )
            self.scores = torch.load(self.scores_path, map_location=torch.device("cpu"))
            self.eval()
        else:
            raise FileNotFoundError(
                f"No saved autoencoder model found in directory {self.root_dir}"
            )
    ...
    def save_autoencoder(self, verbose=True):
        ...
        if verbose:
            print(f"saving autoencoder to {self.root_dir}")
    ...
    def entropy(img, bins=4):
        ...
        dx = img[2:] - img[:-2]
        dx = np.concatenate([np.zeros_like(dx[:1]), dx, np.zeros_like(dx[:1])], axis=0)
        dy = img[:, 2:] - img[:, :-2]
        dy = np.concatenate(
            [np.zeros_like(dy[:, :1]), dy, np.zeros_like(dy[:, :1])], axis=1
        )

        # compute the 2D histogram, i.e. the joint pdf of dx and dy
        hist, xedges, yedges = np.histogram2d(
            dx.flatten(), dy.flatten(), bins=bins, range=[[-1, 1], [-1, 1]]
        )
        hist /= hist.sum()
    ...
    def score(
        self, data: torch.Tensor, reconstructions: torch.Tensor, reduce_batch_dim=True
    ):
    ...
    def rate(self, data):
        ...
        assert (
            len(data.shape) == 4
        ), "data must be a tensor of shape ([batch,] channels, height, width)"
        ...
        percentiles = (
            torch.tensor(
                [
                    percentileofscore(self.scores.cpu(), score.item(), kind="weak")
                    for score in scores
                ]
            )
            / 100
    ...
    def classify(
        self, data, low_threshold=0.33, high_threshold=0.66, entropy_threshold=0.15
    ) -> Union[str, List[str]]:
        """
        ...
        classes = [
            c - 1 if c > LOW and e > entropy_threshold else c
            for (e, c) in zip(entropies, classes)
        ]
    ...
    def csv_to_img(path: str, *args, max_rows=100_000, **kwargs) -> torch.Tensor:
        ...
        try:
            ...
            sheet = ndmap(
                lambda x: datatype_map[type(x)], sheet, shape=shape, dtype=int
            )
            # plt.imshow(sheet); plt.show()

            # convert the datatype indices to one-hot vectors, creating channels for each data type
            sheet = ndmap(
                lambda x: np.eye(len(datatype_list))[x],
                sheet,
                shape=(*shape, len(datatype_list)),
                dtype=np.float32,
            )
...
class SpreadsheetLoader(Dataset):
    ...
    def __init__(
        self, data_root: str, data_suffix=".pt", raw_suffix=".csv", raw_root=None
    ):
        ...
        if raw_root is not None:
            ...
            with Pool(processes=cpu_count()) as pool:
                for _ in tqdm(
                    pool.imap_unordered(
                        save_processed_sheet_data,
                        zip(raw_paths, out_paths),
                        chunksize=1,
                    ),
                    total=len(raw_paths),
                    desc="saving preprocessed data",
                ):
                    pass
        ...
        self.sheets = [
            ...
            for path in tqdm(
                self.paths, desc="loading data", unit="sheets", total=len(self.paths)
            )
        ]
